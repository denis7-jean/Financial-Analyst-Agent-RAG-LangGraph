...
def test_table_chunk_is_preserved_as_html_without_truncation() -> None:
    ...
    assert table_chunk.page_content == TABLE_HTML
    ...
    assert "Gross margin" in table_chunk.page_content
...
def test_chunk_metadata_is_assigned_with_sequential_chunk_ids() -> None:
    ...
    table_chunk = next(chunk for chunk in chunks if chunk.metadata["chunk_kind"] == "table")
